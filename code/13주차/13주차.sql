...
-- JOIN
SELECT m.member_name, c.class_name
...
INSERT INTO members2(member_name, phone, join_date)
SELECT
	'Member_' || g,
...
-- Planning Time: 0.121 ms => 2.061 ms
-- Execution Time: 16.848 ms => 0.201 ms

-- INDEX 추가
CREATE INDEX idx_members_name2
ON members2(member_name);


CREATE VIEW rgistration_view AS
SELECT
	m.member_name AS 회원명,
	c.class_name AS 강좌명,
	r.register_date AS 신청일
FROM registerations r
JOIN members m ON r.member_id = m.member_id
JOIN classes c ON r.class_id = c.class_id;